...
onready var input_keySelection = preload("res://addons/Database_Manager/Scenes and Scripts/Inventory/KeySelect_Template.tscn")
onready var input_spriteDisplay = preload("res://addons/Database_Manager/Scenes and Scripts/Inventory/Sprite_Template.tscn")
...
func add_input_node(index, index_half, i, dict, container1, container2):
	var parent_container = container2
	if index_half < index:
		parent_container = container1
	var node_value =  udsEngine.convert_string_to_type(dict[i]["Value"], dict[i]["DataType"])
...
#				new_field.inputNode.set_text(str(node_value))
		...
		'TYPE_SPRITEDISPLAY':
			...
			var sprite_data = str(udsEngine.current_dict[Item_Name][i]).split("*")
			var sprite_path = sprite_data[0]
			var frameVector : Vector2 = udsEngine.convert_string_to_type(sprite_data[1], "TYPE_VECTOR2")
...
##			var sprite_data = str(input.get_normal_texture().get_path()) + str(frames)
##			print(sprite_data)
##			udsEngine.current_dict[Item_Name][i] = sprite_data
...
func update_item_name(old_name : String, new_name : String = ""):
	if old_name != new_name: #if changes are made to item name
		if !does_key_exist(new_name):
			var item_name_dict = udsEngine.currentData_dict["Row"]
			for i in item_name_dict: #loop through item_row table until it finds the key number for the item
				if item_name_dict[i]["FieldName"] == old_name:
					udsEngine.currentData_dict["Row"][i]["FieldName"] = new_name #Replace old value with new value
					break
...
func add_entry_row(entry_value):
	...
	udsEngine.currentData_dict["Row"][str(item_name_dict_size)] = entry_value
...
func _on_SaveNewItem_button_up():
	...
	if !does_key_exist(Item_Name) and !does_key_contain_invalid_characters(Item_Name) and !has_empty_fields():
		...
		udsEngine.add_key(Item_Name, "TYPE_STRING", true, false, false)
...
func _on_FileDialog_sprite_file_selected(path: String) -> void:
	...
	var sprite_data = str(curr_icon_path).split("*")
	var sprite_path = sprite_data[0]
	var frameVector : Vector2 = udsEngine.convert_string_to_type(sprite_data[1], "TYPE_VECTOR2")
	...
	if udsEngine.is_file_in_folder(udsEngine.icon_folder, new_file_name): #Check if selected folder is Icon folder and has selected file
		udsEngine.current_dict[Item_Name][selected_field_name] = new_file_path + "*" + str(frameVector)
		udsEngine.save_all_db_files(udsEngine.current_table_name)
#		refresh_data(Item_Name)
	else:
#		print("Item selected is NOT in icon folder")
