...
func _ready():
	...
	clear_frozenLines()
	save_path_label.set_text(udsEngine.table_save_path)
	files = udsEngine.list_files_with_param(udsEngine.table_save_path, udsEngine.file_format, ["Table Data.json"])
...
func update_sheet(name):
	...
	udsEngine.currentData_dict["Row"] = {}
	udsEngine.currentData_dict["Column"] = {}
#	udsEngine.current_row_dict = {} #Clears the current selected key dictionary
#	udsEngine.current_column_dict = {} #Clears the current selected field dictionary
	udsEngine.current_table_name = name #*Scirpt Variable* Sets table name
	udsEngine.current_dict = {} #Clears the current selected table dictionary
	...
	for i in udsEngine.currentData_dict["Row"].size():
		...
		row_header_order[udsEngine.currentData_dict["Row"][order_value]["FieldName"]] = order_value
	...
	for n in udsEngine.currentData_dict["Column"].size():
		var order_value = str(n + 1)
		column_header_order[udsEngine.currentData_dict["Column"][order_value]["FieldName"]] = order_value
	...
	match udsEngine.data_type:
		"table":
			...
			for i in column_header_order:
				var newcell_field = column.instance()
			...
			for i in row_header_order:
				var index = 1
				var newcell_key = row.instance()
				table.add_child(newcell_key)
				newcell_key.set_text(str(i))

				# for each row header cell, create data cells *Fields
				for n in udsEngine.currentData_dict["Column"].size():
					var Columnname = udsEngine.currentData_dict["Column"][str(index)]["FieldName"]
					var newcell_value = datainput.instance()
					var y = udsEngine.current_dict[i][Columnname]
...
#Displays the key or field for the selected table
	...
	match udsEngine.data_type:
		"Row":
			var column_count = udsEngine.currentData_dict[udsEngine.data_type]["1"].size() + 1
			...
			for i in udsEngine.currentData_dict[udsEngine.data_type]["1"]:
				var text = str(i)
				if text != "FieldName":
					var newcell_field = column.instance()
					table.add_child(newcell_field)
			...
			for i in udsEngine.currentData_dict[udsEngine.data_type].size():
				var order_value = str(i + 1)
				row_header_order[udsEngine.currentData_dict[udsEngine.data_type][order_value]["FieldName"]] = order_value
			...
			for i in row_header_order:
				...
				for n in udsEngine.currentData_dict[udsEngine.data_type][row_header_order[i]]:
					if n != "FieldName":
						var Columnname = n
						var newcell_value = datainput.instance()
						var field_value = udsEngine.currentData_dict[udsEngine.data_type][row_header_order[i]][n]
						var keyname = udsEngine.remove_special_char(i)
...
#				var node_name = udsEngine.remove_special_char(i)
...
func _on_new_key_Accept_button_up():
	...
	add_key(keyName, datatype, showKey)
...
func _on_DeleteKey_Itemlist_item_selected(index):
	match udsEngine.data_type:
		"Row":
			delete_name = udsEngine.currentData_dict[udsEngine.data_type][str(index + 1)]["FieldName"]
		"Column":
			delete_name = udsEngine.currentData_dict[udsEngine.data_type][str(index + 1)]["FieldName"]
...
func update_table_values(): #input to file
	...
	for i in udsEngine.current_dict:
		var text = udsEngine.remove_special_char(i)
		row_dict[text] = i 
	...
	for i in row_dict:
		var key = row_dict[i]
		for u in udsEngine.current_dict[key]:
			column_dict[udsEngine.remove_special_char(u)] = u
	...
	for e in table.get_children(): #table is a reference to the input field, TableViewer/Gridcontainer
		if e.get_class() == "PanelContainer": #loop through all input PanelContainers and pull data from each node
			var Row = e.row
			var rowname = udsEngine.remove_special_char(Row)
			...
			if e.new_data != "" and e.new_data != null: #check if any of the newdata is blank
				udsEngine.current_dict[Rowname][Columnname] = e.new_data
	...
	refresh_table_data(udsEngine.current_table_name)
...
#							main_tbl[k] = test
		...
		for i in table.get_children():
			if i.get_class() == "PanelContainer":
				...
				if type == "Order":
					if new != "":
						var prev_data_dict = udsEngine.currentData_dict[udsEngine.data_type][prev] #get dict for previous data
						column_tbl[new] = prev_data_dict #replace with new data


		for i in column_tbl: #Remove fields that are going to change
			udsEngine.currentData_dict[udsEngine.data_type].erase(i)
		for i in column_tbl: #update row dictionary with new fields
			udsEngine.currentData_dict[udsEngine.data_type][i] = column_tbl[i]
		...
		for i in table.get_children():
			if i.get_class() == "PanelContainer":
				...
				if type != "Order" and type != "key":
					if new != "":
						var column_order = udsEngine.get_data_index(nm)
						var column_info = udsEngine.currentData_dict[udsEngine.data_type][column_order]
						column_info[type] = new
						udsEngine.currentData_dict[udsEngine.data_type][column_order] = column_info
						i.update_values() #updates all the variables for current node
						i.new_data = ""
...
#					if new == "" or new == null: #if input is blank or null
#						pass
##						udsEngine.row_dict[str(index)] = prev #Set input value of i to previous value (reset value)
#					else:
#						var row_order = udsEngine.get_row_index(prev)
#						udsEngine.current_row_dict[row_order] = new #Update the row dictionary key
#						i.set_name(Newnodename + " key") #Set new name for the current node
#						i.update_values() #updates all the variables for current node
#						i.new_data = ""
#
...
func add_key(keyName, datatype, showKey):
	var new_key_array = udsEngine.get_table_keys()
	var new_field_array = udsEngine.get_table_values()
	#Check for Errors
	error_check()
	if keyName == "": #If new key is blank
		...
		udsEngine.add_key(keyName, datatype, showKey, true, "")
...
func add_newField():
	...
	udsEngine.add_field(fieldName, datatype, showField, required, dropdown)
	_on_newValue_Cancel_button_up()
	update_sheet(udsEngine.current_table_name)
...
func add_table():
	...
	var dup_check = tableName + udsEngine.file_format
	...
	match error: 
		0: #if there are no errors detected
			udsEngine.add_new_table(tableName, keyName, keyDataType, keyVisible, fieldName, fieldDatatype, fieldVisible, dropdown_table)
...
func delete_table(del_tbl_name):
	udsEngine.delete_table(del_tbl_name)
...
func _on_Conver_File_Data_button_up():
	udsEngine.convert_keyFile_to_new_format()
