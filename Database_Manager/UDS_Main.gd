...
onready var tabTemplate = preload("res://addons/Database_Manager/Scenes and Scripts/Tab_Template.tscn")
onready var navButton = preload("res://addons/Database_Manager/Scenes and Scripts/Navigation_Button.tscn")
...
func create_tabs():
	clear_data_tabs()
	clear_buttons()
	var tables_dict = udsEngine.import_data(udsEngine.table_save_path + "Table Data.json")
	for i in tables_dict:
		var create_tab = udsEngine.convert_string_to_type(tables_dict[i]["Create Tab"])
		if create_tab:
