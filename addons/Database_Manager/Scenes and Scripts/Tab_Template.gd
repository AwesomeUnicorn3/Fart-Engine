...
onready var input_vector = preload("res://addons/Database_Manager/Scenes and Scripts/Inventory/Input_Vector.tscn")
...
#				new_field.inputNode.set_text(str(node_value))
		...
		'TYPE_SPRITEDISPLAY':
			...
			var sprite_data = current_dict[Item_Name][i]
			var sprite_path = sprite_data[0]
			var frameVector : Vector2 = convert_string_to_type(sprite_data[1], "TYPE_VECTOR2")
...
#	var input = dict[i]
#	current_node.table_name = current_table_name
...
#			else:
#				input.set_text(current_dict[Item_Name][i])
#		"Dropdown List":
#			current_node.populate_list()
#			var type_id = current_node.get_id(current_dict[Item_Name][i])
...
#
#		"Checkbox":
#			var ckbx_value = convert_string_to_type(current_dict[Item_Name][i])
#			input.pressed = ckbx_value
#
#		"IconDisplay":
#			input.set_text(current_dict[Item_Name]["IconDescription"])
#			var iconPath = current_dict[Item_Name]["IconPath"]
#			if is_file_in_folder(icon_folder, iconPath):
#				current_node.texture_button.set_normal_texture(load(iconPath))
#			else:
#				iconPath = current_dict["Default"]["IconPath"]
#				current_node.texture_button.set_normal_texture(load(iconPath))
#
#		'SpriteDisplay':
#
