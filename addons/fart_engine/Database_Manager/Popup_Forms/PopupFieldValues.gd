...
func add_field_data_to_container():
	#parent_node.data_dict = data_dict
	field_index = get_data_index(fieldName, DatabaseManager.FIELD, data_dict)
	var this_data_dict = data_dict[DatabaseManager.FIELD][field_index]
	var field_type_dict = all_tables_merged_dict["10026"]
	var field_data_type :String = this_data_dict["DataType"]
	var input_node_type_name :String = get_value_as_text(datatype_dict[field_data_type]["Display Name"])

	for child in $PanelContainer/VBox1/Scroll1/VBox1.get_children():
		var node_value = convert_string_to_type(this_data_dict[child.name])
		...
		match child_name:
			...
			"RequiredValue":
				var reqired_value = custom_to_bool(node_value)
			...
			"TableRef":
				...
				var table_ref_name :String = get_value_as_text(child.selection_table[field_table_ref]["Display Name"])
				datatype_input = child
...
func on_text_changed(new_text = "Blank"):
	var selectedName :String = $PanelContainer/VBox1/Scroll1/VBox1/DataType.get_display_name_from_key(int($PanelContainer/VBox1/Scroll1/VBox1/DataType.selectedItemKey))
