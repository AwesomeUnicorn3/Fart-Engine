...
func show_advanced_node(show_node:bool = show_advanced_options_node):
	...
	if !get_tree():
		...
		if labelNode.get_label_text() == "Display Name":
			show_node = false
...
func set_display_value(node_value):
	...
	node_value = convert_string_to_type(str(node_value))

	if typeof(node_value) == TYPE_DICTIONARY:
		display_node.get_node("Display").parse_bbcode(node_value["text"])
	else:
		display_node.get_node("Display").parse_bbcode(node_value)
	print("SET DISPLAY VALUE END")


func _set_input_value(node_value):
	#print("SET TEXT INPUT VALUE BEGIN: ", node_value)
	var text: String
	var node_dict: Variant
	
	node_dict = str_to_var(node_value)
	if typeof(node_dict) !=  27:
		node_dict = {"text": "", "show_advanced_options": true}
		#print(node_dict)
#	else:
#		node_dict = str_to_var(node_value)
	#print("NODE DIXT: ", node_value)
	text = node_dict["text"]
	input_data = node_dict
	$Input_Node/Input.set_text(text)
#
	if !node_dict.has("show_advanced_options"):
		node_dict["show_advanced_options"] = show_advanced_options
	show_advanced_options = node_dict["show_advanced_options"]
#
#
	_on_input_text_changed(text)
	#print("SET TEXT INPUT VALUE END")
	show_advanced_node()
...
#	$Input_Node/AdvancedControlsVBox/AdvancedControlsHBox.visible = show
	...
	$Input_Node/AdvancedControlsVBox/Label/HBox1/Hide_Button.set_text_value(showhide_dict[show])
