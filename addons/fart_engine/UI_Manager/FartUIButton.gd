...
func _init():
	...
	default_button_texture = load(await get_default_button_texture())
...
func get_default_button_texture() -> String:
	...
	if Engine.is_editor_hint():
		UI_scenes_table= DatabaseManager.all_tables_merged_dict["10044"]
	else:
		UI_scenes_table= await FART.import_data("10044")
...
func get_default_background_color() -> Color:
	...
	if Engine.is_editor_hint():
		...
		UI_scenes_table = FART.import_data("10044")
...
func _on_texture_button_up():
	var function_name :String = FART.get_value_as_text(ui_navigation_dict[str(selected_method_key)]["Function Name"])
...
func set_texture(buttonTexture:Texture2D=null, textureLayer:String = "Normal"):
	if buttonTexture == null:
		buttonTexture = load(await get_default_button_texture()) #preload("res://Data/png/AwesomeUnicorn (copy).png")
...
#		if displayName == "":
		if !Engine.is_editor_hint():
			ui_navigation_dict = FART.Static_Game_Dict["10045"]
			if displayName == "":
				displayName= FART.get_value_as_text(ui_navigation_dict[str(selected_method_key)]["Button Label"])
#			label.set_text(displayName)
		else:

			ui_navigation_dict = DatabaseManager.all_tables_merged_dict["10045"]
			if displayName == "":
				displayName = DatabaseManager.get_value_as_text(ui_navigation_dict[str(selected_method_key)]["Button Label"])
