...
@onready var interaction_shader_material :ShaderMaterial = preload("res://addons/fart_engine/Event_Manager/2dOutline_Material.tres")
...
##------------------#ANIMATION SCRIPTS############
func play_sprite_animation_static_or_editor(sprite_texture_data: Dictionary) -> void:
...
func set_default_animation(sprite_texture_data, animation_player) ->void:
	var sprite_atlas_dict :Dictionary = sprite_texture_data["atlas_dict"]
	var sprite_advanced_dict :Dictionary = sprite_texture_data["advanced_dict"]
...
func set_editor_or_static_animation(sprite_texture_data:Dictionary) -> void:
...
func set_editor_or_static_shadow() -> void:
...
func add_sprite_and_collision() -> void:
...
func _on_event_draw():
	if event_name != "":
		...
		event_dict = DatabaseManager.all_events_dict[event_name]
		active_page = "1"
#		print("refresh")
...
func ready_ingame():
	...
	await set_initial_in_game_values()
...
func create_event_data_dict():
	if DBENGINE.save_game_data_dict[current_map_key].has(name) and DBENGINE.save_game_data_dict[current_map_key][name] == {}:
		...
		var dialog_table_name: String = "10021"
		DBENGINE.save_game_data_dict[current_map_key][name][dialog_table_name] = DBENGINE.import_data(dialog_table_name)["1"]
		DBENGINE.save_game_data_dict[current_map_key][name][dialog_table_name].erase("Display Name")
		DBENGINE.save_game_data_dict[current_map_key][name][dialog_table_name] = {"input_dict": DBENGINE.save_game_data_dict[current_map_key][name]["Event Dialog Variables"]}
		DBENGINE.save_game_data_dict[current_map_key][name][dialog_table_name]["options_dict"] = {"action": "4"}
...
func get_condition_value(conditions_dict:Dictionary, condition:String, fieldID:String):
	...
	if table_ID == "Local Variables":
		...
		var options_button_variables_dict = DBENGINE.convert_string_to_type(DBENGINE.save_game_data_dict[current_map_key][name]["10021"]["input_dict"])
...
func get_global_variable_index_name_from_display_name(If_Key_Name_DropDown):
	var global_variable_dict :Dictionary = DBENGINE.Dynamic_Game_Dict["10029"]
	var index
...
func set_interaction_area(sprite_texture_data:Dictionary):
	interaction_area = FART.create_event_interaction_area("Default Animation", sprite_texture_data)
