...
@onready var btn_itemselect = load("res://addons/Database_Manager/Scenes and Scripts/UI_Input_Scenes/Btn_ItemSelect.tscn")
@onready var event_editor_input_form = load("res://addons/Event_Manager/Event Editor Input Form.tscn")
...
#		if currentData_dict["Row"][item_number]["FieldName"] != "Default":
		...
		#!!!!!!!!!!!!!THE BELOW LINE OF CODE CAUSES THE ENTIRE ENGINE TO CRASH!!!!
