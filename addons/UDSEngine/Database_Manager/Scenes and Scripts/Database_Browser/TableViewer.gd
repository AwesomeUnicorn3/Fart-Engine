...
@onready var column = load("res://addons/UDSEngine/Database_Manager/Scenes and Scripts/Database_Browser/Column.tscn")
@onready var row = load("res://addons/UDSEngine/Database_Manager/Scenes and Scripts/Database_Browser/Row.tscn")
@onready var datainput = load("res://addons/UDSEngine/Database_Manager/Scenes and Scripts/Database_Browser/DataInput.tscn")
...
#			table_data = tabledata_json.result
			table_data = DBENGINE.import_data(DBENGINE.main_page.save_path + DBENGINE.table_name + DBENGINE.file_format)
...
#			row_header_order = rowdata_json.result
			row_header_order = DBENGINE.import_data(DBENGINE.main_page.save_path + DBENGINE.table_name + "_Row")
...
#			column_header_order = columndata_json.result
			...
			for i in column_header_values:
				columnCount += 1
				var newcell = column.instantiate()
			...
			for i in row_header_values:
				...
				for n in column_header_values.size():
					...
					var newinput = datainput.instantiate()
...
func update_sheet_RC():
	...
	#create row header cells
	for i in row_header_values:
		var newcell = datainput.instantiate()
