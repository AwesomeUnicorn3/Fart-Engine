...
var default_button_texture:Texture2D = preload("res://Data/png/AU3Button.png")
...
func _on_texture_button_up():
	var function_name :String = AU3ENGINE.convert_string_to_type(ui_navigation_dict[str(selected_method_key)]["Function Name"])["text"]
	AU3ENGINE.UIENGINE.callv(function_name, [])
	reset_self_modulate()
...
#	if is_instance_valid(get_tree()):
		...
		if !get_tree().get_edited_scene_root(): 
			ui_navigation_dict = AU3ENGINE.Static_Game_Dict["UI Script Methods"]
			var displayName :String = AU3ENGINE.convert_string_to_type(ui_navigation_dict[str(selected_method_key)]["Button Label"])["text"]
